...
- **Technique:** two SVG practitioners move through six poses grouped into four conceptual stages. Desktop/tablet scroll advances the sequence. Playback and manual stage selection take precedence over incidental layout scrolling until the visitor intentionally scrolls again.
- **Finish variations:** replays choose between stylized chave de braço, triângulo and mata-leão endings, each with its own transition and final pose. A shuffled cycle prevents immediate repeats; the live caption names the finish. These remain conceptual, not instruction.
